...
export default function DashboardLayout() {
    ...
    useEffect(() => {
        ...
        return () => { socket.off("meeting_ended", handler); };
    }, [socket, refreshMeetings]);

    const openCreateMeetingModal = useCallback(() => setShowCreateMeeting(true), []);
    const openLocationModal = useCallback((address: string) => setLocationModalAddress(address), []);
    const openPoll = useCallback((meetingId: string) => setPollMeetingId(meetingId), []);

    const handleCreateMeeting = useCallback(async (meetingData: any): Promise<AppMeeting | null> => {
        try {
            const res = await fetchWithAuth(`${API_BASE}/meetings`, {
                method: "POST",
    ...
    const shortcuts = useMemo(() => [
        {
            key: "k", mod: true, allowInInput: true, handler: () => {
    ...
    useEffect(() => {
        ...
        if (!label) {
            if (location.pathname.startsWith("/meetings/") || location.pathname.startsWith("/rooms/")) label = "Live Meeting";
            else if (location.pathname.startsWith("/archives/")) label = "Archive";
            else label = "Concord";
        }
