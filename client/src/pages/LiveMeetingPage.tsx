...
function shouldShowMeetingLocation(meeting: any): boolean {
    return (
        (meeting.modality === "Offline" || meeting.modality === "Hybrid") &&
        Boolean(String(meeting.location || "").trim())
    );
}
...
export default function LiveMeetingPage({
    ...
    const [pinnedChatMessage, setPinnedChatMessage] =
        useState<ChatMessage | null>(null);
    const [transcripts, setTranscripts] = useState<any[]>([]);
    const [tasks, setTasks] = useState<any[]>([]);
    const [liveParticipants, setLiveParticipants] = useState<any[]>([]);
    const [dockOpen, setDockOpen] = useState(true);
    const [dockActivePanelId, setDockActivePanelId] =
        useState<DockPanelId>("agenda");
    ...
    useEffect(() => {
        if (isPersonalRoom && roomId) {
            ...
            fetchWithAuth(`${API_BASE}/meetings/personal-room/${roomId}`)
                ...
                .then((data) => {
                    if (data && (data._id || data.id)) setSelectedMeeting(data);
                })
                .catch((err) =>
                    console.error("Failed to fetch personal room:", err),
                );
            return;
        }
        ...
        fetchWithAuth(`${API_BASE}/meetings/${meetingId}`)
            ...
            .catch((err) =>
                console.error("Failed to fetch meeting by link:", err),
            );
    }, [
        meetingId,
        roomId,
    ...
    useEffect(() => {
        ...
        const refreshed = meetings.find(
            (m) => resolvedInternalMeetingId(m) === id,
        );
        if (refreshed && refreshed !== selectedMeeting)
            setSelectedMeeting(refreshed);
    }, [meetings, selectedMeeting]);

    const isHost =
        String(selectedMeeting?.hostId || "") ===
        String(user?.id || user?._id || "");
    const isOffline = selectedMeeting?.modality === "Offline";
    const [callJoined, setCallJoined] = useState(false);
    const chatSessionActive = Boolean(
        selectedMeeting && (isOffline || callJoined),
    );
    ...
    const fetchAgenda = useCallback(
        async (id: string) => {
            try {
                const res = await fetchWithAuth(`${API_BASE}/agenda/${id}`);
    ...
    const fetchTranscript = useCallback(
        async (id: string) => {
            try {
                const res = await fetchWithAuth(`${API_BASE}/transcript/${id}`);
    ...
    const fetchChat = useCallback(
        async (id: string) => {
            ...
            try {
                    ...
                    return {
                        ...
                        ...(sys
                            ? {
                                  system: sys,
                                  presenceIsSelf:
                                      String(msg.senderId) === currentUserIdStr,
                              }
                            : {}),
                    };
                };
                ...
                const pinnedRaw = Array.isArray(raw)
                    ? null
                    : (raw.pinned ?? null);

                setPinnedChatMessage(
                    pinnedRaw ? normalizeMsg(pinnedRaw) : null,
                );

                setChatMessages((prev) => {
                    if (chatFetchMeetingRef.current !== id) return prev;
                    const byId = new Map<string, ChatMessage>(
                        history.map((m) => [m.id, m]),
                    );
                    for (const m of prev) {
    ...
    useEffect(() => {
        ...
        const handleChatMessage = (msg: any) => {
            ...
            const clientMsgId =
                msg.clientMsgId != null ? String(msg.clientMsgId) : null;
            setChatMessages((prev) => {
                if (clientMsgId) {
        ...
        const handleChatPresence = (payload: any) => {
            ...
            const isSelfPresence =
                String(payload.userId ?? "") === currentUserIdStr;
            const row: ChatMessage = {
                id,
                meetingId: meetingIdStr,
                senderId: String(payload.userId ?? ""),
                senderName: isSelfPresence
                    ? "You"
    ...
    const dockShortcuts = useMemo(
        () => [
            ...
            { key: "i", handler: () => selectDockPanel("minutes") },
        ],
        [selectDockPanel],
    );
    ...
    const minutesTasks = useMemo(
        () =>
            tasks.filter(
                (t) =>
                    !t.assigneeId && (!t.assignees || t.assignees.length === 0),
            ),
        [tasks],
    );
    ...
    const handleAddMinute = useCallback(
        async (title: string) => {
            ...
            try {
                ...
                console.error("Failed to create minute item:", err);
            }
        },
    ...
    const handleAgendaChange = useCallback(
        async (items: any[]) => {
            ...
            if (!id) return;
            try {
                await fetchWithAuth(`${API_BASE}/agenda/${id}`, {
                    method: "POST",
    ...
    const upcomingMeetings = useMemo(
        () =>
            meetings
                .filter((m) => !isMeetingCompletedByTime(m, nowTs))
            ...
            <div className="page-shell">
                <header className="page-header">
                    ...
                    <p className="page-header-description">
                        Select an upcoming meeting to join the call, agenda,
                        transcript, chat, and tasks.
                    </p>
                </header>
                <div className="meeting-list">
                    {upcomingMeetings.map((meeting) => (
                        <div
                            key={
                                resolvedInternalMeetingId(meeting) ?? meeting.id
                            }
                        ...
                        >
                            ...
                            <div className="meeting-card-meta">
                                <span
                                    className={`chip ${meeting.modality === "Online" ? "chip-blue" : meeting.modality === "Hybrid" ? "chip-purple" : "chip-emerald"}`}
                                >
                                    {meeting.modality}
                                </span>
                                <span
    ...
    return (
        <div
            ...
            className={`meeting-layout ${isOffline ? "offline-mode" : ""} ${!dockOpen ? "dock-collapsed" : ""}`}
        >
            <VideoArea
                meetingId={internalMeetingId}
                ...
                onRefreshTasks={() =>
                    internalMeetingId && fetchTasks(internalMeetingId)
                }
