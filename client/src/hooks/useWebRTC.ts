		...
		try {
			...
			if (!isSharing) {
				await activeRoom.localParticipant.setScreenShareEnabled(true, {
					// When audio is wanted, disable browser voice-processing so
					// music / system sounds aren't mangled by echo cancellation /
					// noise suppression. `systemAudio: 'include'` prompts Chrome
					// to show the "Share system audio" checkbox — without it the
					// option may never appear and capture silently fails.
					audio: screenShareSystemAudio
						? { echoCancellation: false, noiseSuppression: false, autoGainControl: false }
						: false,
					...(screenShareSystemAudio && { systemAudio: 'include' }),
