...
function HotkeyDisplay({
    ...
    hotkey?: { key: string; mod?: boolean; shift?: boolean; alt?: boolean };
}) {
...
export default function CommandPalette({
    ...
    const rankedActions = useMemo(() => {
        ...
        results.sort(
            (a, b) =>
                a.score - b.score || a.action.name.localeCompare(b.action.name),
        );
    ...
    useEffect(() => {
        ...
        debounceRef.current = setTimeout(async () => {
            ...
            try {
                ...
                if (latestQueryRef.current !== q) return;
                setMeetingResults(
                    meetingsRes.ok ? await meetingsRes.json() : [],
                );
    ...
    const executeMeeting = useCallback(
        (m: any, transcriptText?: string) => {
            onClose();
    ...
    const executePerson = useCallback(
        (p: any) => {
            // Navigate to profile or search for their meetings
            onClose();
            navigate(
                `/archives?participant=${encodeURIComponent(p.name || p.email)}`,
            );
        },
        [navigate, onClose],
    );
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <span className="cp-row-right">
                                            <span className="cp-badge cp-badge-group">
                                                {action.group}
                                            </span>
                    ...
                    {showApiSection && (
                        <>
                                ...
                                <div className="cp-section">
                                    ...
                                    {meetingResults.map((m, i) => {
                                        ...
                                        const isSelected =
                                            gIdx === selectedIndex;
                                        const snippets: any[] =
                                            m.matchedTranscripts?.slice(0, 2) ??
                                            [];
                                        return (
                                            <div
                                                key={m._id ?? i}
                                                className="cp-meeting-group"
                                            >
                                                <button
                                                    type="button"
                                                    data-idx={gIdx}
                                                ...
                                                >
                                                    ...
                                                    <span className="cp-row-right">
                                                        <span
                                                            className={`cp-badge ${m.status === "completed" ? "cp-badge-archive" : "cp-badge-meeting"}`}
                                                        >
                                                            {m.status ===
                                                            "completed"
                                                                ? "Archive"
                                                ...
                                                {snippets.map((t, si) => (
                                                    ...
                                                    >
                                                        <span className="cp-snippet-speaker">
                                                            {t.speaker ||
                                                                "Unknown"}
                                                        </span>
                                                        <span className="cp-snippet-text">
                                                            {truncate(
                                                                t.text,
                                                                90,
                                                            )}
                                                        </span>
                                                        <span className="cp-snippet-arrow">
                                                            → transcript
                                                        </span>
                                                    </button>
                                                ))}
                                ...
                                <div className="cp-section">
                                    ...
                                    {personResults.map((p, i) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <span
                                                    className="cp-row-icon cp-avatar-icon"
                                                    style={{
                                                        ["--avatar-hue" as any]:
                                                            hue,
                                                    }}
                                                >
                                                    {p.profileImage ? (
                                                        ...
                                                        <span className="cp-avatar-initials">
                                                            {initials}
                                                        </span>
                                                    )}
                                                </span>
                            ...
                            {!searchLoading &&
                                !hasApiResults &&
                                rankedActions.length === 0 && (
                                    <div className="cp-empty cp-empty-full">
