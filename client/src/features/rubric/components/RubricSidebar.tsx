...
export default function RubricSidebar({
    ...
    const handleScore = async () => {
        ...
        try {
            const res = await (fetchWithAuth || fetch)(
                `${API_BASE}/rubric/${meetingId}/evaluate`,
                {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                        participantId:
                            selectedParticipant._id || selectedParticipant.id,
    ...
    const handleExportReport = () => {
        window.open(
            `${API_BASE}/rubric/${meetingId}/report?format=html`,
            "_blank",
        );
    };
                        ...
                        showUpload ? (
                            <div>
                                <p className="rubric-hint">
                                    Paste JSON array or CSV (name, maxScore,
                            ...
                            >
                                <Icon icon={Add01Icon} size={12} /> Upload
                                Rubric
                            </button>
                        )
                    ) : (
                                ...
                                <div className="rubric-scoring-mb">
                                    ...
                                    {selectedParticipant &&
                                        rubric.criteria.map((c, i) => (
                                            <div
                                                key={i}
                                                className="rubric-criterion-gap"
                                            >
                                                <label className="rubric-criterion-label">
                                                    {c.name}
                                                </label>
                                    ...
                                    {selectedParticipant && (
                                        <button
                                            className="btn btn-sm btn-primary rubric-submit-mt"
                                            onClick={handleScore}
                                        >
                                            Submit Scores
                                        </button>
