...
import {
    ...
    STATUS_LOOKUP,
    CATEGORY_TEXT_COLOR,
} from "../../dashboard/components/ArchiveTaskTable";
...
export default function Tasks({
    ...
    const getItemHostId = (item: Task) =>
        String(item.meetingHostId || meetingHostId || "");
    const isHostForItem = (item: Task) =>
        Boolean(currentUserId) && getItemHostId(item) === currentUserId;
    ...
    const canEditStatus = (item: Task) =>
        isHostForItem(item) ||
        (isAssigneeForItem(item) && item.status !== "verified");
    const canCreateItems =
        Boolean(meetingId) &&
        Boolean(currentUserId) &&
        String(meetingHostId || "") === currentUserId;

    const agendaLookup = useMemo(() => {
        return new Map<string, string>(
            agendaItems.map((item) => [item.id, item.title]),
        );
    }, [agendaItems]);
    ...
    const archiveParticipants = useMemo(
        () => toArchiveParticipants(participants || []),
        [participants],
    );
    ...
    useEffect(() => {
        const hasSelectedAgenda =
            newAgendaItemId &&
            agendaItems.some((item) => item.id === newAgendaItemId);
    ...
    const groupedSections = useMemo(() => {
        ...
        const groups = agendaItems
            .map((agendaItem) => ({
                key: agendaItem.id,
                title: agendaItem.title,
                items: items.filter(
                    (item) => item.agendaItemId === agendaItem.id,
                ),
            }))
        ...
        const unlinkedItems = items.filter(
            (item) =>
                !item.agendaItemId ||
                !agendaLookup.has(String(item.agendaItemId)),
        );
    ...
    const handleDelete = async (itemId: string) => {
        try {
            await (fetchWithAuth || fetch)(`${API_BASE}/tasks/${itemId}`, {
                method: "DELETE",
            });
            onRefresh?.();
        } catch (err) {
    ...
    const resetFields = () => {
        ...
        if (participants && participants.length > 0) {
            setNewAssigneeIds([
                String(participants[0]._id || participants[0].id),
            ]);
        } else {
            setNewAssigneeIds([]);
        }
        ...
        try {
            ...
            if (newAssigneeIds.length > 0) {
                const p = (participants || []).find(
                    (pp: any) => String(pp._id || pp.id) === newAssigneeIds[0],
                );
                if (p) {
                    body.assignee = p._id || p.id;
                    body.assigneeName = p.name;
                }
            }
            const res = await (fetchWithAuth || fetch)(
                `${API_BASE}/tasks/${meetingId}`,
                {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                ...
                <div className="tasks-body">
                    <div className="tasks-list">
                            ...
                            <div key={group.key} className="tasks-group">
                                {group.title && (
                                    <div className="tasks-group-header">
                                        <span className="tasks-group-title">
                                            {group.title}
                                        </span>
                                ...
                                {group.items.map((item, index) => {
                                    ...
                                    const allowedStatuses = isHost
                                        ? HOST_STATUSES
                                        : ASSIGNEE_STATUSES;
                                    const isCollapsed =
                                        collapsedCards.has(itemId);
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div
                                                className="live-task-card-header"
                                                style={{
                                            ...
                                            {!isCollapsed && (
                                                <>
                                                    ...
                                                    <div className="live-task-card-field">
                                                        <span className="live-task-card-label">
                                                            Assigned to
                                                        </span>
                                                        {isHost &&
                                                        archiveParticipants.length >
                                                            0 ? (
                                                            <TaskAssigneePicker
                                                                participants={
                                                                    archiveParticipants
                                                        ...
                                                        <div className="live-task-card-field">
                                                            ...
                                                            {isHost ? (
                                                                <input
                                                                    ...
                                                                    defaultValue={formatDateOnlyDisplay(
                                                                        item.deadline,
                                                                    )}
                        ...
                        {canCreateItems && adding ? (
                            <div
                                className="glass-card inline-form-card"
                            ...
                            >
                                ...
                                <div className="inline-form-row">
                                    <select
                                        className="input-field tasks-select-flex"
                                        value={newCategory}
                                        onChange={(e) =>
                                    ...
                                    {archiveParticipants.length > 0 && (
                                        <TaskAssigneePicker
                                            ...
                                            selectedAssignees={newAssigneeIds.map(
                                                (id) => {
                                                    const p =
                                                        archiveParticipants.find(
                                                            (pp) =>
                                                                String(
                                                                    pp._id,
                                                                ) === id,
                                ...
                                {agendaItems.length > 0 && (
                                    <select
                                        className="input-field tasks-select-mb"
                                        value={newAgendaItemId}
                                        onChange={(e) =>
                                            setNewAgendaItemId(e.target.value)
                                        }
                                    >
                                        <option value="">
                                            General / Unlinked
                            ...
                            canCreateItems && (
                                ...
                                >
                                    ...
                                    >
                                        <Icon icon={Add01Icon} size={16} /> Add
                                        Task
                                    </button>
                                </ShortcutTooltip>
                            )
