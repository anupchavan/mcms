...
export default function PollVoting({ meetingId, onClose }: PollVotingProps) {
    ...
    const shareableMeetingUrl = useMemo(
        () =>
            meetingUrl && !pathnameHasMongoMeetingSegment(meetingUrl)
                ? meetingUrl
                : "",
        [meetingUrl],
    );
    ...
    const totalVotes =
        poll?.slots?.reduce((sum, s) => sum + (s.votes?.length || 0), 0) || 0;
    const isResolved = poll?.status === "resolved";

    return (
        <div className="modal-overlay" onClick={onClose}>
            <div
                className="modal-content poll-modal"
                onClick={(e) => e.stopPropagation()}
            >
                <div className="modal-header">
                    <h2 className="modal-title">
                        {isResolved ? "Poll Results" : "Vote on Meeting Time"}
                    </h2>
                            ...
                            {poll.slots.map((slot, i) => {
                                ...
                                const pct =
                                    totalVotes > 0
                                        ...
                                        : 0;
                                const isWinner =
                                    isResolved && poll.resolvedSlot === i;
                                ...
                                return (
                                    <button
                                        ...
                                        onClick={() => {
                                            if (!isResolved) setSelectedSlot(i);
                                        }}
                                        disabled={isResolved}
                                    >
                                        <div
                                            className="poll-slot-bar"
                                            style={{ width: `${pct}%` }}
                                        />
                                        <div className="poll-slot-content">
                                            <div className="poll-slot-left">
                                                <div
                                                    className={`poll-slot-radio${isSelected ? " checked" : ""}`}
                                                >
                                                    {isSelected && (
                                                        <Icon
                                                            icon={Tick01Icon}
                                                ...
                                                <div className="poll-slot-info">
                                                    ...
                                                    <span className="poll-slot-time">
                                                        {slot.time}
                                                    </span>
                                                </div>
                                            </div>
                                            <div className="poll-slot-right">
                                                <span className="poll-slot-votes">
                                                    {voteCount} vote
                                                    {voteCount !== 1 ? "s" : ""}
                                                </span>
                                                {(hasVoted || isResolved) && (
                                                    <span className="poll-slot-pct">
                                                        {pct}%
                                                    </span>
                                                )}
                                                {isWinner && (
                                                    <span className="poll-slot-winner-badge">
                                                        Winner
                                                    </span>
                                                )}
                                            </div>
