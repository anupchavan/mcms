...
export default function AttendanceMarkPage({
    ...
    const canMarkAttendance = useMemo(
        () => Boolean(meetingId && token && user?.token),
        [meetingId, token, user?.token],
    );
        ...
        const markAttendance = async () => {
            ...
            try {
                const res = await fetch(
                    `${API_BASE}/attendance/${meetingId}/mark`,
                    {
                        method: "POST",
                        headers: {
                ...
                if (res.status === 401) {
                    ...
                    setMessage(
                        data.message ||
                            "Your session expired. Please sign in again and rescan the QR code.",
                    );
                    return;
                }
    ...
    return (
        <div className="attendance-page-root">
            <div className="attendance-page-card">
                <div
                    className="attendance-icon-circle"
                ...
                <p className="attendance-page-sub">{message}</p>

                {!user?.token && (
                    <p className="attendance-page-note">
                        Once you sign in, this page will automatically finish
