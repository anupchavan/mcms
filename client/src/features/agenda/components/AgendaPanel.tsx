...
const AgendaPanel: FC<AgendaPanelProps> = ({
    agendaItems = [],
    isHost = false,
    onItemChange,
}) => {
                ...
                {items.length === 0 && !isAdding ? (
                    ...
                    items.map((item) => (
                        <div
                            ...
                            title={
                                isHost
                                    ? "Click to change status (Pending -> Active -> Completed)"
                                    : "Only the host can update agenda items"
                            }
                        ...
                        >
                            ...
                            <div className="agenda-item-flex">
                                ...
                                <div className="agenda-item-duration">
                                    {item.duration} mins
                                </div>
                            </div>
                            <div
                                className={`chip chip-xs chip-${item.status === "active" ? "blue" : item.status === "completed" ? "emerald" : "amber"}`}
                            >
                                {item.status}
                            </div>
                        </div>
