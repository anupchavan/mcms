...
export default function QROverlay({
    ...
    const fetchReport = useCallback(async () => {
        ...
        try {
            const res = await fetch(
                `${API_BASE}/attendance/${meetingId}/report`,
                {
    ...
    const generateQR = useCallback(async () => {
        ...
        try {
            setError(null);
            const res = await fetch(
                `${API_BASE}/attendance/${meetingId}/generate-qr`,
                {
                    method: "POST",
                    headers: {
                ...
                <div className="qr-left-col">
                    ...
                    <div className="qr-caption">
                        ...
                        <p className="qr-countdown">
                            Auto-refreshes in{" "}
                            <span className="qr-countdown-value">
                ...
                <div className="qr-right-col">
                    <div className="qr-right-header">
                        ...
                        <div className="qr-right-header-actions">
                            <div className="chip chip-emerald">
                                ...
                                {report?.total || 0} Present
                            </div>
                            <button
                                onClick={onClose}
                                className="btn btn-secondary qr-close-btn"
                            >
                    ...
                    <div className="qr-attendee-list">
                        {report?.attended?.length > 0 ? (
                            report.attended.map((a: any, i: number) => (
                                <div key={i} className="qr-attendee-row">
                                    <span className="qr-attendee-name">
                                        {a.user?.name ||
                                            a.user?.email ||
                                            "Unknown User"}
                                    </span>
                                    <span className="qr-attendee-time">
                                        {new Date(
                                            a.joinTimestamp,
                                        ).toLocaleTimeString([], {
                                            hour: "2-digit",
