...
function VideoTile({
    ...
    const {
        ...
        border: colorBorder,
        text: colorText,
    } = getAvatarCssVars(hue, isDark);
    const initials = getInitials(name || "");
    const profileImageUrl = profileImage
        ? `${SERVER_BASE}${profileImage}`
        : null;
    ...
    const videoObjectFit =
        videoObjectFitProp ?? (isScreenShare ? "contain" : "contain");
    ...
    const noVideo =
        !isScreenShare &&
        (cameraOn !== undefined ? !cameraOn : videoMuted || !hasVideo);
    ...
    useEffect(() => {
        if (!noVideo && videoRef.current?.paused) {
            videoRef.current.play().catch(() => {
                /* autoplay policy: ignored, LiveKit handles audio */
            });
        }
    });
    return (
            ...
            style={
                {
                    ...
                    background:
                        noVideo && !profileImageUrl ? colorBg : undefined,
                } as CSSProperties
            }
        >
            <button
                ...
                <video
                    ...
                    style={
                        isSelf
                            ? {
                                  transform: "scaleX(-1)",
                                  display: noVideo ? "none" : undefined,
            ...
            {audioMuted && !isScreenShare && (
                <div
                    className="video-tile-muted-badge"
...
export default function VideoArea({
        ...
        try {
            const res = await (fetch as any)(`${API_BASE}/tasks/${meetingId}`, {
                ...
                body: JSON.stringify({
                    title,
                    category: type,
                    agendaItemId: activeAgendaItem?.id || null,
                }),
            });
            if (res.ok) {
                onRefreshTasks?.();
    ...
    const advanceAgenda = () => {
        const activeIdx = agendaItems.findIndex(
            (item) => item.status === "active",
        );
        if (activeIdx === -1) {
            // If none active, make first pending active
            const firstPendingIdx = agendaItems.findIndex(
                (item) => item.status === "pending",
            );
            if (firstPendingIdx !== -1) {
                const newItems = [...agendaItems];
                newItems[firstPendingIdx] = {
                    ...newItems[firstPendingIdx],
                    status: "active",
                };
            ...
            const nextPendingIdx = newItems.findIndex(
                (item, idx) => idx > activeIdx && item.status === "pending",
            );
            if (nextPendingIdx !== -1) {
                newItems[nextPendingIdx] = {
                    ...newItems[nextPendingIdx],
                    status: "active",
                };
            }
    ...
    useEffect(() => {
        ...
        if (currentUser) {
            list.push({
                _id: (currentUser as any).id || (currentUser as any)._id,
        ...
        peers.forEach((p) => {
            ...
            if (
                p.userId !== (currentUser as any)?.id &&
                p.userId !== (currentUser as any)?._id
            ) {
                seenPeer.add(uid);
        ...
        () => [
            ...
            },
            {
                ...
                handler: () =>
                    isHost &&
                    hasJoined &&
                    hostControlsRef.current?.toggleRecording(),
                allowInInput: false,
            },
            {
                key: "c",
                handler: () => hasJoined && toggleVideo(),
                allowInInput: false,
            },
            {
                key: "a",
                handler: () => isHost && onTriggerAddAgendaItem?.(),
                allowInInput: false,
            },
            {
                key: "a",
                shift: true,
                handler: () => isHost && onTriggerAddTask?.(),
                allowInInput: false,
            },
            {
                key: "Enter",
                handler: () => !hasJoined && canJoin && handleJoin(),
                allowInInput: false,
            },
            {
                key: "l",
            ...
            // End meeting shortcut only fires for the host
            {
                ...
                handler: () =>
                    isHost &&
                    hasJoined &&
                    hostControlsRef.current?.endMeeting(),
                allowInInput: false,
            },
        ],
        [
    ...
    const handleScreenAspect = useCallback((tileId: string, ratio: number) => {
        setScreenAspects((prev) => {
            ...
            if (existing && Math.abs(existing - ratio) / ratio < 0.005)
                return prev;
    ...
    const galleryFallbackCols = Math.max(
        1,
        Math.ceil(Math.sqrt(meetingTiles.length)),
    );
                ...
                <div className="va-info-row">
                    <div className="va-info-text">
                        ...
                        <div className="video-meeting-meta">
                            <span
                                className={`offline-dot ${connected ? "connected" : ""}`}
                            />
                            ...
                            {modality === "Offline" && (
                                <span className="chip chip-emerald chip-xs">
                                    Offline Focus
                                </span>
                            )}
                        </div>
                    ...
                    {modality === "Offline" ? (
                                            ...
                                            {agendaItems.length === 0 ? (
                                                ...
                                                agendaItems
                                                    ...
                                                    .map((item, idx) => (
                                                        <div
                                                            key={item.id}
                                                            className={`focus-agenda-item ${item.status}`}
                                                        >
                                                            <div
                                                                className={`focus-ai-num ${item.status}`}
                                                            >
                                                                {item.status ===
                                                                "completed" ? (
                                                                    <Icon
                                                            ...
                                                            <div className="focus-ai-content">
                                                                <div className="focus-ai-name">
                                                                    {item.title}
                                                                </div>
                                                                <div className="focus-ai-time">
                                                                    {
                                                                        item.duration
                                                                    }{" "}
                                                                    min ·{" "}
                                                            ...
                                                            <span
                                                                className={`focus-ai-status st-${item.status}`}
                                                            >
                                                                {item.status}
                                                            </span>
                                                        </div>
                                    ...
                                    <div className="focus-progress-wrap">
                                        ...
                                        <div className="focus-pb-track">
                                            <div
                                                ...
                                                style={{
                                                    width: `${(agendaItems.filter((i) => i.status === "completed").length / (agendaItems.length || 1)) * 100}%`,
                                                }}
                                            ></div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="focus-section">
                                        ...
                                        <div className="focus-note-card">
                                            <textarea
                                                ...
                                                placeholder={
                                                    isHost
                                                        ? "Type a note, decision, or task..."
                                                        : "Type a note or parking-lot item..."
                                                }
                                                value={activeNote}
                                                onChange={(e) =>
                                                    setActiveNote(
                                            ...
                                            <div className="focus-note-actions">
                                                ...
                                                <button
                                                    className="focus-note-btn primary"
                                ...
                                <div className="focus-right">
                                    <div className="focus-section">
                                        ...
                                        <div className="focus-attendee-row">
                                            {participants?.map((p, idx) => (
                                                <div
                                                    key={p.id || idx}
                                                    className="focus-av-chip"
                                                >
                                                    <div className="focus-av-dot">
                                                        {p.name?.charAt(0) ||
                                                            "U"}
                                                    </div>
                                                    <span className="focus-av-name">
                                                        {p.name || "User"}
                                                    </span>
                                                </div>
                                            ))}
                                            {participants?.length === 0 && (
                                                <span className="va-empty-text">
                                                    No attendees listed
                                                </span>
                                            )}
                                        </div>
                                        ...
                                        <div className="focus-action-feed">
                                            {tasks.length === 0 ? (
                                                ...
                                                tasks.map((ai, idx) => (
                                                    <div
                                                        key={ai.id || idx}
                                                        className={`focus-action-card ${ai.category?.toLowerCase()}`}
                                                    >
                                                        <div className="focus-ac-top">
                                                            <span
                                                                className={`focus-ac-type ${ai.category?.toLowerCase()}`}
                                                            >
                                                                {ai.category}
                                                            </span>
                                                            <span className="focus-ac-assignee">
                                                                →{" "}
                                                                {ai.assignee ||
                                                                    "Unassigned"}
                                                            </span>
                                                        </div>
                                                        <div className="focus-ac-text">
                                                            {ai.title}
                                                        </div>
                                                    </div>
                                                ))
                                    ...
                                    <div className="focus-section">
                                        ...
                                        <div className="focus-parking-lot">
                                            <div className="focus-pl-title">
                                                Off-topic items
                                            </div>
                                            {parkingLotItems.length === 0 ? (
                                                <div className="va-empty-label">
                                                    Empty
                                                </div>
                                            ) : (
                                                parkingLotItems.map(
                        ...
                        <div className="video-prejoin">
                            <div className="prejoin-card">
                                ...
                                <button
                                    className="btn btn-primary prejoin-btn"
                                    onClick={handleJoin}
                                    disabled={!canJoin}
                                >
                                    {mediaError ? (
                                ...
                                style={
                                    hasAnyScreenShare
                                        ? ({
                                              ...
                                              "--filmstrip-size":
                                                  stageLayout?.filmstripPlacement ===
                                                  "none"
                                                      ? "0px"
