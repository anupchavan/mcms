...
export default function HotkeyRecorder({ value, onChange, checkConflict }: HotkeyRecorderProps) {
    const [recording, setRecording] = useState(false);
    const [liveMods, setLiveMods] = useState({ mod: false, shift: false, alt: false });
    ...
    useEffect(() => {
        ...
        function onKeyDown(e: KeyboardEvent) {
            ...
            if (e.key === 'Meta' || e.key === 'Control') { setLiveMods(p => ({ ...p, mod: true })); return; }
            if (e.key === 'Shift') { setLiveMods(p => ({ ...p, shift: true })); return; }
            if (e.key === 'Alt') { setLiveMods(p => ({ ...p, alt: true })); return; }
        ...
        function onKeyUp(e: KeyboardEvent) {
            ...
            if (cur) {
                ...
                if (conflict) {
                    ...
                    if (e.key === 'Meta' || e.key === 'Control') setLiveMods(p => ({ ...p, mod: false }));
                    else if (e.key === 'Shift') setLiveMods(p => ({ ...p, shift: false }));
                    else if (e.key === 'Alt') setLiveMods(p => ({ ...p, alt: false }));
                    return;
                }
    ...
    }, [recording, cancelRecording, clearConflictCountdown, startConflictCountdown]);

    useEffect(() => {
        if (!recording) return;
        function onPointerDown(e: PointerEvent) {
            const pop = document.querySelector('.hotkey-popover');
            if (pop?.contains(e.target as Node) || triggerRef.current?.contains(e.target as Node)) return;
            cancelRecording();
        }
    ...
    if (isConflictLocked && conflictName) {
        ...
        statusLabel = conflictName ? `Already used by "${conflictName}"` : 'Release to save';
    } else if (hasAnything) {
    ...
    return (
        <>
            <div className="hotkey-recorder">
                ...
                >
                    {value?.key
                        ? formatHotkey(value).split(' ').map((p, i) => (
                            <span key={i} className="hotkey-chip-part">{p}</span>
                        ))
                        : <span className="hotkey-chip-placeholder">Record</span>
                    }
                </button>
                {value?.key && (
                    <button type="button" className="hotkey-clear-btn" onClick={() => { onChange(undefined); cancelRecording(); }} title="Clear">×</button>
                )}
            </div>

            {recording && popoverPos && createPortal(
                <div
                ...
                >
                    <button type="button" className="hotkey-popover-close" onClick={cancelRecording}>×</button>

                    <div className="hotkey-popover-key-display">
                        {hasAnything
                            ? keycaps.map((part, i) => (
                                <span key={i} className={`hotkey-popover-keycap${(conflictName || isConflictLocked) ? ' hotkey-popover-keycap-conflict' : ''}`}>
                                    {part}
                                </span>
                            ))
                            : <span className="hotkey-popover-keycap hotkey-popover-keycap-empty">—</span>
                        }
                    </div>

                    <span className={`hotkey-popover-label${(conflictName || isConflictLocked) ? ' hotkey-popover-label-conflict' : ''}`}>
                        {statusLabel}
                    </span>
                </div>,
                document.body
