...
export default function ConfigureActions() {
    const { getAlias, getHotkey, savePreferences, loading } =
    ...
    const [localHotkeys, setLocalHotkeys] = useState<
        Record<string, HotkeyDef | undefined>
    >({});
    const [editingAliasId, setEditingAliasId] = useState<string | null>(null);
    const [savedIndicator, setSavedIndicator] = useState(false);
    const [expandedGroups, setExpandedGroups] = useState<Set<string>>(
        new Set(ACTION_GROUPS),
    );
    ...
    const handleAliasChange = useCallback(
        (id: string, v: string) => setLocalAliases((p) => ({ ...p, [id]: v })),
        [],
    );
    ...
    const handleHotkeyChange = useCallback(
        (id: string, hotkey: HotkeyDef | undefined) => {
    ...
    const resetAction = useCallback(
        (action: ActionDef) => {
            setLocalAliases((p) => ({
                ...p,
                [action.id]: action.defaultAlias,
            }));
            setLocalHotkeys((p) => ({
                ...p,
                [action.id]: action.defaultHotkey,
            }));
            scheduleSave();
        },
        [scheduleSave],
    ...
    const getAliasConflict = useCallback(
        (actionId: string): string | undefined => {
            ...
            const ownerAction = ALL_ACTIONS.find(
                (a) =>
                    a.id !== actionId &&
                    a.scope === scope &&
    ...
    if (loading)
        return (
            <div className="configure-actions-loading">
                Loading preferences…
            </div>
        );
        ...
        <div className="configure-actions">
            <div className="configure-actions-header">
                ...
                <p className="configure-actions-subtitle">
                    Set aliases and keyboard shortcuts. Type an alias in{" "}
                    <kbd>{isMac ? "⌘" : "Ctrl"} K</kbd> to run any action.
                </p>
                {savedIndicator && (
                    <span className="configure-actions-saved">Saved ✓</span>
                )}
            </div>
                ...
                {ACTION_GROUPS.map((group) => {
                    ...
                    return (
                        <div key={group} className="configure-actions-group">
                            ...
                            >
                                <span
                                    className={`configure-actions-chevron${expanded ? " expanded" : ""}`}
                                >
                                    <Icon icon={ArrowRight01Icon} size={14} />
                                </span>
                                <span className="configure-actions-group-name">
                                    {group}
                                </span>
                            </button>
                            ...
                            {expanded &&
                                actions.map((action, idx) => {
                                    const alias =
                                        localAliases[action.id] ??
                                        action.defaultAlias;
                                    const hotkey = localHotkeys[action.id];
                                    const isEditing =
                                        editingAliasId === action.id;
                                    const hk = hotkeyKey(hotkey);
                                    const scopedHk = hk
                                        ? `${action.scope}:${hk}`
                                        : "";
                                    const conflictOwner = scopedHk
                                        ? hotkeyOwners[scopedHk]
                                        : undefined;
                                    const isHotkeyConflict =
                                        conflictOwner &&
                                        conflictOwner !== action.name;
                                    const aliasConflictName = getAliasConflict(
                                        action.id,
                                    );
                                    const isDefaultAlias =
                                        alias === action.defaultAlias;
                                    const isDefaultHotkey = !hotkey?.key
                                        ? !action.defaultHotkey?.key
                                        : hotkey.key ===
                                              action.defaultHotkey?.key &&
                                          !!hotkey.mod ===
                                              !!action.defaultHotkey?.mod &&
                                          !!hotkey.shift ===
                                              !!action.defaultHotkey?.shift &&
                                          !!hotkey.alt ===
                                              !!action.defaultHotkey?.alt;
                                    const isDefault =
                                        isDefaultAlias && isDefaultHotkey;
                                    ...
                                    return (
                                        ...
                                        >
                                            {/* Name */}
                                            <div className="configure-actions-col configure-actions-col-name">
                                                <span className="configure-actions-action-name">
                                                    {action.name}
                                                </span>
                                            </div>
                                            ...
                                            <div className="configure-actions-col configure-actions-col-alias">
                                                {isEditing ? (
                                                    <input
                                                        ...
                                                        title={
                                                            aliasConflictName
                                                                ? `Already used by "${aliasConflictName}"`
                                                                : undefined
                                                        }
                                                    />
                                                ) : (
                                                    <button
                                                        ...
                                                        title={
                                                            aliasConflictName
                                                                ? `Alias conflict: already used by "${aliasConflictName}"`
                                                                : undefined
                                                        }
                                                    >
                                                        {alias || (
                                                            <span className="alias-placeholder">
                                                                —
                                                            </span>
                                                        )}
                                                    </button>
                                            ...
                                            <div className="configure-actions-col configure-actions-col-hotkey">
                                                <HotkeyRecorder
                                                    ...
                                                    checkConflict={checkHotkeyConflict(
                                                        action.id,
                                                    )}
                                                />
                                            </div>
